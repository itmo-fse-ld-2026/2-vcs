...
class GitGraphMapper(GraphMapper):
  def __init__(self, client: IFMOPortalClient, asker: CLIAsker, users: List[User], logger: Logger, work_dir: str):
  ...
  def _ensure_repo_exists(self):
    if not os.path.exists(os.path.join(self.work_dir, ".git")):
      ...
      self.logger.log(f"git init {self.work_dir}")
  ...
  def process_branch_create(self, branch_id: int, from_branch_id: Optional[int]):
    ...
    if from_branch_id is None:
      ...
      self.logger.log(f"git checkout -b {branch_name}")
    else:
  ...
  def process_branch_switch(self, branch_id: int):
    ...
    self.logger.log(f"git checkout {branch_name}")
